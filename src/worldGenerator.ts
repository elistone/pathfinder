...
export class WorldGenerator {
            ...
            for (let j = 0; j < distance; j++) {
                ...
                if (cell) {
                    ...
                    if (this.rng.randomBoolean(0.4)) {
                        ...
                        const wideCell = this.grid.getCell({x: currentX + offsetX, y: currentY + offsetY});
                        if (wideCell && this.isWithinBounds(currentX + offsetX, currentY + offsetY)) {
                            wideCell.reset();
                        }
                    }
                }
     ...
     */
    private floodFill(x: number, y: number, visited: boolean[][], reachableCells: Position[]): void {
    ...
    private generateCaves(): Position[][] {
        ...
        for (let i = 0; i < numCaves; i++) {
            ...
            const caveX = divX * divWidth + Math.floor(this.rng.random() * (divWidth - caveWidth) * 0.8);
            const caveY = divY * divHeight + Math.floor(this.rng.random() * (divHeight - caveHeight) * 0.8);

            // Ensure cave is within bounds
            if (caveX > 0 && caveY > 0 && caveX + caveWidth < width && caveY + caveHeight < height) {
                const cavePoints = this.generateCave(caveX, caveY, caveWidth, caveHeight);
                if (cavePoints.length > 0) {
                    caveRegions.push(cavePoints);
                    console.log(`Cave generated at (${caveX},${caveY}) with size ${caveWidth}x${caveHeight}, ${cavePoints.length} open cells`);
                }
            }
        }
     ...
     */
    private smoothCave(startX: number, startY: number, endX: number, endY: number): void {
    ...
    private connectCaves(caves: Position[][]): void {
        ...
        for (let i = 0; i < caves.length - 1; i++) {
            ...
            if (currentCave.length > 0 && nextCave.length > 0) {
                ...
                console.log(`Creating tunnel from (${start.x},${start.y}) to (${end.x},${end.y})`);
    ...
    private connectToStart(caves: Position[][]): void {
        ...
        console.log(`Connecting start to closest cave at (${closestPoint.x},${closestPoint.y})`);
    ...
    private createTunnel(start: Position, end: Position): void {
        ...
        while (currentX !== end.x || currentY !== end.y) {
            ...
            const distIfMoveX = Math.abs(end.x - (currentX + Math.sign(end.x - currentX))) +
                Math.abs(end.y - currentY);
            const distIfMoveY = Math.abs(end.x - currentX) +
                Math.abs(end.y - (currentY + Math.sign(end.y - currentY)));

            // Pick the move that gets us closer, with some randomness
            const moveX = this.rng.random() < 0.8 ?
            ...
            if (this.rng.random() < 0.7) {
                ...
                if (this.rng.random() < 0.5) {
                    const wideCell1 = this.grid.getCell({x: currentX + widthX, y: currentY});
                    const wideCell2 = this.grid.getCell({x: currentX, y: currentY + widthY});
                    ...
                    if (wideX) {
                        const wideCell = this.grid.getCell({x: currentX + widthX, y: currentY});
                        if (wideCell && this.isWithinBounds(currentX + widthX, currentY)) {
                            wideCell.reset();
                        }
                    } else {
                        const wideCell = this.grid.getCell({x: currentX, y: currentY + widthY});
