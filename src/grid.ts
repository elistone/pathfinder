...
export class Grid {
  ...
  public moveViewport(deltaX: number, deltaY: number): void {
    ...
    const newViewportX = Math.max(
      0,
      Math.min(this.worldWidth - this.viewWidth, this.viewportX + deltaX),
    );
    const newViewportY = Math.max(
      0,
      Math.min(this.worldHeight - this.viewHeight, this.viewportY + deltaY),
    );
  ...
  public worldToViewPosition(worldPos: Position): Position | null {
    ...
    if (
      viewX >= 0 &&
      viewX < this.viewWidth &&
      viewY >= 0 &&
      viewY < this.viewHeight
    ) {
  ...
  public isWithinViewport(worldPosition: Position): boolean {
    return (
      worldPosition.x >= this.viewportX &&
